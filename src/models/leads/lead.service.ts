...
export class LeadService {
	...
	async find() {
		...
		const leadsData = (await this.repository.find(leadParams)) as ScanItemOutput;
		...
		for (const leadItem of leadsData.Items) {
			...
			const interestsData = (await this.repository.query(interestsParam)) as QueryItemOutput;
			result[leadItem.id] = {
				...leadItem,
